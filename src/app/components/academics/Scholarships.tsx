...
export default function Scholarships() {
  ...
  return (
    <div className="mb-10">
      ...
      <p
        className={`pt-8 text-[#2E2E2E] ${language === 'en' ? 'font-raleway' : 'font-preahvihear'}`}
      >
        {language === 'en' ? (
          <>
            We believe that financial barriers should not stand in the way of a
            quality education. Our department is committed to helping students
            achieve their academic goals through a variety of scholarship
            opportunities and financial aid options
          </>
        ) : (
          <>
            យើងជឿថា​ឧបសគ្គហិរញ្ញវត្ថុមិនគួរជាបញ្ហាដែលរារាំងការទទួលបានការអប់រំដែលមានគុណភាពទេ។
            នាយកដ្ឋានរបស់យើងមានប្តេជ្ញាផ្តល់ជំនួយដល់និស្សិតក្នុងការសម្រេចគោលដៅសិក្សារបស់ពួកគេ
            តាមរយៈឱកាសអាហារូបករណ៍និងជម្រើសជំនួយហិរញ្ញវត្ថុជាច្រើន។
          </>
        )}
      </p>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4 text-sm mt-10">
              <div className="bg-[#767676]/10 rounded-md p-2 w-full">
                <p
                  className={`font-medium text-gray-900 ${language === 'en' ? 'font-reddit_sans' : 'font-preahvihear'}`}
                >
                  {language === 'en' ? 'Duration' : 'រយៈពេល'}
                </p>
                <p
                  className={`font-medium text-gray-900 py-3 ${language === 'en' ? 'font-reddit_sans' : 'font-preahvihear'}`}
                >
                  {language === 'en'
                    ? `${sch.en.duration}`
                    : `${sch.kh.duration}`}
            ...
            <div className="mt-4 text-sm bg-[#767676]/10 p-3 rounded-md mb-5">
              <p
                className={`font-semibold text-gray-900 mb-2 ${language === 'en' ? 'font-reddit_sans' : 'font-preahvihear'}`}
              >
                {language === 'en'
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              ...
              <div className="mt-4 text-xs">
                <p
                  className={`font-semibold text-[#3A3B5C] mb-1 text-base ${language === 'en' ? 'font-reddit_sans' : 'font-preahvihear'}`}
                >
                  {language === 'en'

...
const Footer: React.FC = () => {
  ...
  const headingFont = language === 'en' ? 'font-raleway' : 'font-preahvihear';
  const bodyFont = language === 'en' ? 'font-raleway' : 'font-kantumruy_pro';
    ...
    <footer className="w-full bg-[#3A3B5C] text-[#FFFFFF] font-raleway overflow-hidden lg:py-12 py-5">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          ...
          <div>
            <h3
              className={`text-[clamp(1.25rem,2vw,1.5rem)] font-medium mb-4 ${headingFont}`}
            >
              {footerItems.departmentName}
            </h3>
            <p
              className={`text-[clamp(0.875rem,1vw,1rem)] font-normal mb-6 leading-relaxed ${bodyFont}`}
            >
          ...
          {/* Quick Links + Academic Resources */}
          <div className="flex flex-row gap-8 sm:flex-col md:flex-row">
            {/* Quick Links */}
              ...
              <ul className="space-y-2">
                {footerItems.quickLinks.map((link, index) => (
                  <li key={index}>
                    <a
                      href="#"
                      className={`text-[clamp(0.875rem,1vw,1rem)] hover:underline ${bodyFont}`}
                    >
                      {link}
                    </a>
                  </li>
            ...
            <div className="flex-1">
              <h3
                className={`text-[clamp(1.25rem,2vw,1.5rem)] font-medium mb-4 ${headingFont}`}
              >
                {language === 'en' ? 'Academic Resources' : 'ធនធានសិក្សា'}
              </h3>
              <ul className="space-y-2">
          ...
          <div className="md:col-span-2 xl:col-span-1">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center">
                ...
                <p className={`text-[clamp(0.875rem,1vw,1rem)] ${bodyFont}`}>
              ...
              <div className="flex items-start">
                ...
                <div className="flex flex-col text-[clamp(0.875rem,1vw,1rem)] space-y-1">
                  {footerItems.contact.emails.map((email, idx) => (
                    <a
                      key={idx}
                      href={`mailto:${email}`}
                      className={`hover:underline ${bodyFont}`}
                    >
                      {email}
                    </a>
                  ))}

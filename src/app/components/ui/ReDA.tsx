...
const clubs = [
  {
    title: { en: "LLM Application and Research Club", kh: "សមាគមស្រាវជ្រាវ និងកម្មវិធី LLM" },
    desc: {
  ...
  {
    title: { en: "Computer Vision and AI Club", kh: "សមាគមកុំព្យូទ័រចម្រង់ និង AI" },
    desc: {
  ...
  {
    title: { en: "Data Science and Machine Learning Club", kh: "សមាគមវិទ្យាទិន្នន័យ និងការសិក្សាម៉ាស៊ីន" },
    desc: {
  ...
  {
    title: { en: "Data and Business Analytics Club", kh: "សមាគមវិភាគទិន្នន័យ និងអាជីវកម្ម" },
    desc: {
  ...
  return (
    <section className="bg-white py-10 min-h-screen flex flex-col items-center gap-y-5">
      ...
      <div className="text-center max-w-3xl px-4 space-y-4">
        <h2 className={`text-3xl md:text-4xl font-bold text-gray-900 ${language === "en" ? "font-playfair_display" : "font-preahvihear"}`}>
          {language === "en" ? "Research and Data Analytics Lab" : "មន្ទីរស្រាវជ្រាវ និងវិភាគទិន្នន័យ"}
        </h2>
        <p className={`text-gray-600 text-base leading-relaxed ${language === "en" ? "font-raleway" : "font-kantumruy_pro"}`}>
          {language === "en"
      ...
      >
        {clubs.map((club, idx) => (
          <SwiperSlide key={idx} className="transition-transform duration-300 h-auto min-h-[400px] content-start">
            {({ isActive }) => (
              <div
                className={`p-6 rounded-xl shadow-md border-2 bg-white flex flex-col justify-between transition-transform duration-300 h-auto min-h-[300px] mt-8 ${
                  club.color
                } ${isActive ? "scale-105 shadow-xl" : "scale-95 opacity-80"}`}
              >
                <div>
                  <h3 className={`text-lg font-medium text-gray-900 ${language === "en" ? "font-playfair_display" : "font-preahvihear"}`}>
                    {language === "en" ? club.title.en : club.title.kh}
                  </h3>
                  <p className={`text-gray-600 mt-3 text-sm ${language === "en" ? "font-raleway" : "font-kantumruy_pro"}`}>
                    {language === "en" ? club.desc.en : club.desc.kh}
                  </p>
                </div>
                ...
                <div className="flex justify-end">
                  <a
                    href="#"
                    className={`text-[#C41E3A] text-sm font-semibold hover:underline mt-1 ${language === "en" ? "font-raleway" : "font-kantumruy_pro"}`}
                  >
                    {language === "en" ? "Learn More →" : "សូមស្វែងយល់បន្ថែម →"}
                  </a>
                </div>
              </div>
            )}
      ...
      {/* Collaborate with Us */}
      <div className="bg-[#3A3B5C] text-white rounded-xl p-6 shadow-md flex flex-col items-center min-w-[1200px] mt-12">
        <h3 className={`text-2xl font-semibold mb-2 ${language === "en" ? "font-raleway" : "font-kantumruy_pro"}`}>
          {language === "en" ? "Collaborate with Us" : "សហការជាមួយយើង"}
        </h3>
        <p className={`mb-4 text-sm text-center ${language === "en" ? "font-raleway" : "font-kantumruy_pro"}`}>
          {language === "en"
        ...
        <div className="flex justify-center">
          <button className="bg-white text-indigo-900 px-4 py-2 rounded-full font-medium hover:bg-gray-100 transition">
            <p className={`inline-flex font-bold ${language === "en" ? "font-raleway" : "font-kantumruy_pro"}`}>
              {language === "en" ? "Contact ReDA Lab" : "ទាក់ទង ReDA Lab"} <span><ArrowRight className="w-10"/></span>
            </p>
          </button>
        </div>
      </div>

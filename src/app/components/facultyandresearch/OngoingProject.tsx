  ...
  return (
    <section className="w-full">
      <h1 className="text-3xl font-playfair_display text-black font-semibold">
        {renderTextWithFont(language === "en" ? "Ongoing Projects" : "គម្រោងកំពុងដំណើរ", language, "heading")}
      </h1>
      ...
      {ongoingproject.map((item: OngoingProject, index: number) => {
                ...
                >
                  <div className="p-4 text-gray-700 font-raleway">
                    <div className="text-[#2E2E2E]/80">
                      <p className="mb-2">
                        <strong>{renderTextWithFont(language === "en" ? "Project Duration:" : "រយៈពេលគម្រោង:", language, "body")}</strong>{" "}
                        {item.duration}
                      </p>
                      <p className="mb-2">
                        <strong>{renderTextWithFont(language === "en" ? "Principal Investigators:" : "អ្នកស្រាវជ្រាវសំខាន់:", language, "body")}</strong>{" "}
                        {item.investigators.join(", ")}
                      </p>
                      <p className="mb-2">
                        <strong>{renderTextWithFont(language === "en" ? "Industry Partner:" : "ដៃគូឧស្សាហកម្ម:", language, "body")}</strong>{" "}
                        {item.partner}
                      </p>
                      <p className="mb-2">
                        <strong>{renderTextWithFont(language === "en" ? "Funding:" : "ថវិកាសហការ:", language, "body")}</strong>{" "}
                        {item.funding}
                      </p>
                      <p className="mb-2">
                        <strong>{renderTextWithFont(language === "en" ? "Student Team:" : "ក្រុមបច្ចុប្បន្ន:", language, "body")}</strong>{" "}
                        {item.students}
                      </p>
                    </div>
                    <p className="text-[#2E2E2E]/80">{renderTextWithFont(item.description, language, "body")}</p>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>

...
export default function ContactPage() {
  ...
  const text = {
    ...
    kh: {
      ...
      collaborateText:
        'ចាប់អារម្មណ៍ក្នុងការសហការស្រាវជ្រាវ ឬឱកាសដៃគូ? យើងស្វាគមន៍ការទំនាក់ទំនងជាមួយដៃគូឧស្សាហកម្ម រដ្ឋាភិបាល និងស្ថាប័នសិក្សាផ្សេងៗ។',
  ...
  // font helpers
  const headingFont =
    language === 'kh' ? 'font-preahvihear' : 'font-playfair_display';
  const bodyFont = language === 'kh' ? 'font-kantumruy_pro' : 'font-raleway';

  // size helpers
  const headingSize =
    language === 'kh' ? 'text-xl md:text-2xl' : 'text-2xl md:text-3xl';
    ...
    <div className="w-full">
      ...
      <div className="container mx-auto px-6 lg:px-20 py-16 grid grid-cols-1 lg:grid-cols-3 gap-12 w-full">
        ...
        <div className="col-span-2">
          <h2
            className={`${headingFont} ${headingSize} font-semibold text-black`}
          >
            {t.getInTouch}
          </h2>
          ...
          <form className="space-y-4 mt-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4">
              <div>
                <label
                  className={`${bodyFont} text-black text-lg font-semibold`}
                >
                  {t.firstName}
                </label>
                <input
        ...
        <div className="col-span-1">
          ...
          <div className="space-y-6 bg-white shadow-2xl rounded-xl p-6 border border-[#C41E3A] mt-10 border-l-8">
            <div className="flex items-start space-x-2">
              ...
              <div>
                <h4
                  className={`${language === 'kh' ? 'font-kantumruy_pro' : 'font-playfair_display'} text-lg text-[#2E2E2E] font-medium`}
                >
                  {t.address}
                </h4>
          ...
          <div className="mt-8 bg-[#3A3B5C] text-white rounded-xl p-6 shadow-md flex flex-col items-center space-y-3">
            <h3
              className={`${headingFont} ${language === 'kh' ? 'text-lg' : 'text-xl'} font-semibold`}
            >
              {t.collaborateTitle}
            </h3>
            <p className={`${bodyFont} mb-4 text-center text-sm`}>
              {t.collaborateText}
            </p>
      ...
      <div className="container mx-auto px-6 lg:px-20 pb-16">
        <div className="bg-gray-100 rounded-xl py-6 px-10 shadow-2xl border border-[#3A3B5C] border-l-8">
          <h3
            className={`${headingFont} ${headingSize} mb-2 text-black inline-flex items-center ${language === 'kh' ? 'font-medium' : 'font-semibold'}`}
          >
            <Building className="mr-5" /> {t.visitTitle}
          </h3>
          <p className={`${bodyFont} mb-4 text-sm text-gray-800`}>
            {t.visitText}
          </p>
